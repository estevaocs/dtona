...
(
    SELECT  
        ...
        AVG( t5.review_score) AS avg_review_score, -- média do score de reviews
        ...
        CAST( COUNT( DISTINCT STRFTIME('%m', t1.order_approved_at)) AS FLOAT) / MIN(1 + CAST(t3.idade_base / 30 AS INTEGER), 6) AS prop_ativacao, -- proporcao de meses em que o vendedor  vendeu

        SUM(CASE WHEN JULIANDAY(t1.order_estimated_delivery_date) < JULIANDAY(t1.order_delivered_customer_date) THEN 1 ELSE 0 END) / COUNT( DISTINCT t2.order_id) AS atraso, -- proporcao de pedidos em atraso
        CAST( AVG(JULIANDAY(t1.order_estimated_delivery_date) - JULIANDAY(t1.order_purchase_timestamp)) AS INTEGER) AS avg_tempo_etrega_est, -- tempo médio de entrega prevista
        ...
        SUM(t2.price) / COUNT(DISTINCT STRFTIME('%m', t1.order_approved_at)) AS avg_vl_venda_mes_ativado, -- receita média por mes em que o vendedor estava ativo
        ...
        COUNT(t2.product_id) / COUNT(DISTINCT t2.order_id) AS avg_qtd_produto_venda, -- media de produto vendido por venda
